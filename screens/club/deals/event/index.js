...
const EventScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (event) {
      ...
      // Filter tickets based on criteria - ONLY VIP TICKETS
      const vipTickets = event.ticket_types.filter((ticket) => {
        return ticket.is_rahaclub_vip === true;
      });
      setFilteredTickets(vipTickets); // Set the filtered VIP tickets
    }
  }, [event]);
  ...
  const handleCreateInvoice = async () => {
    ...
    for (const [ticketTypeId, ticketInfo] of Object.entries(
      ...
      if (ticketInfo.quantity > 0) {
        const ticketType = filteredTickets.find(
    ...
    try {
      ...
      navigation.navigate("EventDealCheckout", { checkoutData });
  ...
  return (
    <SafeAreaView>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {filteredTickets.length > 0 && ( // Use filteredTickets here - now VIP tickets only
          <View style={styles.ticketsSection}>
            <Text style={styles.ticketsTitle}>Available Deals</Text>{" "}
            {/* Updated title */}
            {filteredTickets.map(
              (
                ticket,
                index // Map over filteredTickets (VIP tickets)
              ) => (
                <TicketSelection
                  key={ticket.id}
                  ticket={ticket}
